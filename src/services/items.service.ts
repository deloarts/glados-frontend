...
export class BoughtItemsService {
    ...
    getItems(params: any) {
        console.log("Items cache before response:", Array.from(this.cache.values()));
        if (this.cache.has(cacheKeyBoughtItems)) {
            ...
            if ((Date.now() - cachedItems.date.getTime()) < constants.fetchBoughtItemsAfter) {
        ...
        return boughtItemsRequest.getItems(params).then(items => {
            ...
            console.log("Items cache after response:", Array.from(this.cache.values()));
            return items;
        })
    }
